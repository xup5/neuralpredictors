...
class MultiCore(Core, nn.Module):
    """
    Combine multiple core into one (catenet outputs).
    """

    def __init__(
        self,
        core_configs, # A list of core_config dictionaries.
        core_types, # A list of core classes.
        ):
        ...
        for core_config, core_type in zip(self.core_configs, self.core_types):
            self.cores.append(core_type(**core_config))
        self.cores = nn.ModuleList(self.cores)

    def initialize(self):
        """
        Initialization applied on the cores.
        """
    ...
    def forward(self, input_):
        ret = []
        for core in self.cores:
            ret.append(core.forward(input_))
        return torch.cat(ret, dim=1)

    def regularizer(self):
        reg = 0
        for core in self.cores:
            reg = reg + core.regularizer()
        return reg
...
# class spatial_field(nn.Module):
#     def __init__(self, size_in):
#         super(spatial_field, self).__init__()

#         self.size_in = size_in
#         x_scale = torch.Tensor(0.01)
#         self.x_scale = nn.Parameter(x_scale)  # nn.Parameter is a Tensor that's a module parameter.
#         y_scale = torch.Tensor(0.01)
#         self.y_scale = nn.Parameter(y_scale)

#     def forward(self, x):
#         w_times_x= torch.mm(x, self.weights.t())
#         return torch.add(w_times_x, self.bias)  # w times x + b

class my_Stacked2dCore(Core, nn.Module):
    """
    An instantiation of the Core base class. Made up of layers layers of nn.sequential modules.
    Allows for the flexible implementations of many different architectures, such as convolutional layers,
    or self-attention layers.
...
class my_SE2dCore(my_Stacked2dCore, nn.Module):
    """
    An extension of the Stacked2dCore class. The convolutional layers can be set to be either depth-separable
    (as used in the popular MobileNets) or based on self-attention (as used in Transformer networks).
    Additionally, a SqueezeAndExcitation layer (also called SE-block) can be added after each layer or the n final
